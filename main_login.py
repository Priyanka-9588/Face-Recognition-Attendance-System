...
class Login_Window:
    def __init__(self,root):
        ...
        # myname=Label(self.root,text="Developed By: Priyanka",fg="black",bg="white",font=("times new roman",18,"bold"))
    ...
    def login(self):
        if self.txtuser.get()=="" or self.txtpass.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",user="root",password="1234",database="facial_recognition")
            ...
            if row==None:
                ...
                # open_main=messagebox.askyesno("Yes No","Access only Authority Person",parent=self.root)
                # if open_main>0:
                        self.new_window=Toplevel(self.root)
    ...
    def forgot_password_window(self):
        if self.txtuser.get()=="":
            ...
            if row==None:
                messagebox.showerror("My Error","Plaese enter the valid username")
            else:
                conn.close()
                self.root2=Toplevel()
                ...
                self.combo_securiy_Q["values"]=("Select","Your Birth Place","Your school/college name","Your Pet Name")
...
class Register:
    def __init__(self,root):
        ...
        self.checkbtn=Checkbutton(frame,variable=self.var_check,text="I Agree to The Terms & Conditions",bg='white',font=("times new roman",12,"bold"),onvalue=1,offvalue=0)
